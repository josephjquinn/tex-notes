...
\begin{homeworkProblem}
	\textbf{(1) Likelihood and Negative Log-Likelihood.}

	For binary labels $y^{(i)} \in \{0,1\}$ and nonlinear feature map $\varphi(x)$, the model is
	\[
		f(x) = \sigma(w^\top \varphi(x)), \quad \sigma(z) = \frac{1}{1+e^{-z}}.
	\]

	The likelihood for a single data point is
	\[
		p(y^{(i)} \mid x^{(i)}, w) = \big(f(x^{(i)})\big)^{y^{(i)}} \big(1 - f(x^{(i)})\big)^{1-y^{(i)}}.
	\]
	...
	Log-likelihood:
	\[
		\ell(w) = \sum_{i=1}^N \left[ y^{(i)} \log f(x^{(i)}) + (1-y^{(i)}) \log (1-f(x^{(i)})) \right].
	\]
	...
	This makes sense because it is the Bernoulli likelihood of seeing the observed labels given predicted probabilities.

	\vspace{0.3cm}
	\textbf{(2) Convexity of NLL.}

	Gradient of NLL:
	\[
		\nabla_w \text{NLL}(w) = \sum_{i=1}^N \big(f(x^{(i)}) - y^{(i)}\big)\,\varphi(x^{(i)}).
	\]

	Hessian:
	\[
		\nabla_w^2 \text{NLL}(w) = \sum_{i=1}^N f(x^{(i)})(1-f(x^{(i)}))\,\varphi(x^{(i)}) \varphi(x^{(i)})^\top.
	\]

	Each term is positive semidefinite since $f(x^{(i)})(1-f(x^{(i)})) \geq 0$ and $\varphi(x^{(i)}) \varphi(x^{(i)})^\top$ is PSD.
	Therefore, $\text{NLL}(w)$ is convex.

	\vspace{0.3cm}
	\textbf{(3) Gradient Descent Update.}

	With learning rate $\epsilon$, the update rule is
	\[
		w \;\leftarrow\; w - \epsilon \,\nabla_w \text{NLL}(w),
	\]
	which expands to
	\[
		w \;\leftarrow\; w - \epsilon \sum_{i=1}^N \big(f(x^{(i)}) - y^{(i)}\big)\,\varphi(x^{(i)}).
	\]
\end{homeworkProblem}

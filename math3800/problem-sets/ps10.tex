...
\begin{homeworkProblem}
	Let $p$ and $q$ be distinct odd primes with
	\[
		p = q + 4k \qquad (k \in \mathbb{Z}).
	\]
	We show that
	\[
		\left( \frac{a}{p} \right)
	...
	\begin{proof}
		...
		\]
		where $g_p$ (resp.\ $g_q$) is the number of negative representatives of
		\[
			a, 2a, \dots, \frac{p-1}{2}a
			\qquad
			\left(\text{resp.\ } a,2a,\dots,\frac{q-1}{2}a\right)
		\]
		...
		When reduced modulo $p$ or modulo $q$, each of these additional terms appears in
		\emph{pairs} of the form $\pm r$. Such pairs contribute an \emph{even} number of

...
\section{The Golden Rule: Quadratic Reciprocity}

\subsection{Quadratic Congruences}

\begin{manualtheorem}{7.1}
	Let $p$ be a prime and let $a, b, c \in \mathbb{Z}$ with $p \nmid a$.
	Then there exist integers $b_0$ and $c_0$ such that the set of solutions to
	\[
		a x^2 + b x + c \equiv 0 \pmod{p}
	\]
	is equal to the set of solutions to a congruence of the form
	\[
		x^2 + b_0 x + c_0 \equiv 0 \pmod{p}.
	\]
\end{manualtheorem}
\begin{manualtheorem}{7.2}
	Let $p$ be a prime, and let $b$ and $c$ be integers.
	Then there exists a linear change of variable, $y = x + \alpha$ with $\alpha$ an integer, transforming the congruence $x^2 + bx + c \equiv 0 \pmod{p}$ into a congruence of the form $y^2 \equiv \beta \pmod{p}$ for some integer $\beta$.
\end{manualtheorem}

\begin{definition*}
	If $a$ is an integer and $p$ is a prime and $a \equiv b^2 \pmod{p}$ for some integer $b$, then $a$ is called a \emph{quadratic residue modulo $p$}. If $a$ is not congruent to any square modulo $p$, then $a$ is a \emph{quadratic non-residue modulo $p$}.
\end{definition*}

\begin{manualtheorem}{7.7}
	...
	\begin{enumerate}
		...
		\item If $a$ is a quadratic residue modulo $p$ and $b$ is a quadratic non-residue modulo $p$, then $ab$ is a quadratic non-residue modulo $p$.
		\item If $a$ and $b$ are both quadratic non-residues modulo $p$, then $ab$ is a quadratic residue modulo $p$.
	\end{enumerate}

	The last point may seem strange, as the product of two non-squares need not be a square in the integers.
\end{manualtheorem}

\begin{definition*}
	For an odd prime $p$ and a natural number $a$, the \emph{Legendre symbol} $\left( \frac{a}{p} \right)$ is defined by
	\[
		\left( \frac{a}{p} \right) =
		\begin{cases}
			1  & \text{if } a \text{ is a quadratic residue modulo } p,     \\[4pt]
			-1 & \text{if } a \text{ is a quadratic non-residue modulo } p, \\[4pt]
			0  & \text{if } p \text{ divides } a.
		\end{cases}
	\]
\end{definition*}

Theorem~7.7 can be recast as saying that the Legendre symbol is \emph{completely multiplicative}:

\begin{manualtheorem}{7.8}
	Suppose $p$ is an odd prime and $a,b \in \mathbb{Z}$. Then
	\[
...
\begin{manualtheorem}{7.9}[Euler’s Criterion]
	Suppose $p$ is an odd prime and $p$ does not divide the natural number $a$. Then
	\[
		a^{(p-1)/2} \equiv
...
\begin{manualtheorem}{7.12}
	(Infinitude of Primes of the Form $4k+1$).
	There are infinitely many primes congruent to $1 \pmod{4}$.
\end{manualtheorem}

\begin{manualtheorem}{7.13}
	Let $p$ be a prime, let $a$ be an integer not divisible by $p$, and let
	$r_1, r_2, \dots, r_{(p-1)/2}$ be the representatives of
	$a, 2a, \dots, \frac{p-1}{2}a$ in the complete residue system
	\[
		\left\{
	...
	Then
	\[
		r_1 r_2 \cdots r_{(p-1)/2}
		= (-1)^{g} \left( \frac{p-1}{2}! \right),
	\]
	where $g$ is the number of the $r_i$ which are negative.

	\emph{Hint:} It suffices to show that we never have
	$r_i \equiv -r_j \pmod{p}$ for any $i$ and $j$.
\end{manualtheorem}

\begin{manualtheorem}{7.14}
	(Gauss' Lemma).
	Let $p$ be a prime and let $a$ be an integer not divisible by $p$.
	Let $g$ be the number of negative representatives of
	$a, 2a, \dots, \frac{p-1}{2}a$ in the complete residue system
	\[
		\left\{
		-\frac{p-1}{2}, \dots, -1, 0, 1, \dots, \frac{p-1}{2}
...
\section{Pythagorean Triples, Sums of Squares, and Fermat’s Last Theorem}

\begin{definition*}
	A triple of positive integers $(a,b,c)$ satisfying
	\[
		a^{2} + b^{2} = c^{2}
	\]
	is called a \emph{Pythagorean triple}.
\end{definition*}

\begin{manualtheorem}{8.1}
	(Pythagorean Parity Theorem).
	If $(a,b,c)$ is a Pythagorean triple, then at least one of $a$ or $b$ is even.
\end{manualtheorem}

\begin{definition*}{8.2}
	A Pythagorean triple $(a,b,c)$ is called \emph{primitive} if $a$, $b$, and $c$ share no common factor.

\end{definition*}

\begin{manualtheorem}{8.4}
	In any primitive Pythagorean triple, one leg is odd, one leg is even, and the hypotenuse is odd.
\end{manualtheorem}

\begin{manualtheorem}{8.5}
	Let $s$ and $t$ be distinct natural numbers with $s > t$. Then
	\[
		(2st,\; s^{2} - t^{2},\; s^{2} + t^{2})
	\]
...
\begin{manualtheorem}{8.6}
	Let $(a,b,c)$ be a primitive Pythagorean triple where $a$ is the even leg.
	Then
	\[
		\frac{c+b}{2} = s^{2}
		\qquad\text{and}\qquad
		\frac{c-b}{2} = t^{2}
	\]
	for some perfect squares $s^{2}$ and $t^{2}$, and $s$ and $t$ are relatively prime.
\end{manualtheorem}

\begin{manualtheorem}{8.7}[Pythagorean Triple Theorem]
	Let $(a,b,c)$ be a triple of natural numbers with $a$ even, $b$ odd, and $c$ odd.
	Then $(a,b,c)$ is a primitive Pythagorean triple if and only if there exist
	relatively prime positive integers $s$ and $t$, one even and one odd, such that
	\[
		a = 2st,\qquad b = s^{2} - t^{2},\qquad c = s^{2} + t^{2}.
	\]
\end{manualtheorem}

\begin{manualtheorem}{8.10}
	In every primitive Pythagorean triple, one leg is an odd integer greater than $1$
	and the other leg is a positive multiple of $4$.
\end{manualtheorem}

\begin{manualtheorem}{8.11}
	Any odd number greater than $1$ can occur as a leg in a primitive Pythagorean triple.
\end{manualtheorem}

\subsection{Sums of squares}

\begin{manualtheorem}{8.14}
	Let $p$ be a prime such that $p = a^{2} + b^{2}$ for some natural numbers $a$ and $b$. Then either $p = 2$ or $p \equiv 1 \pmod{4}$.
\end{manualtheorem}

\begin{manualtheorem}{8.15}
	Let $p$ be a prime and let $a$ be a natural number not divisible by $p$. Then there exist integers $x$ and $y$ such that
	\[
		ax \equiv y \pmod{p}
		\quad\text{with}\quad
		0 < |x|,\,|y| < \sqrt{p}.
...
\begin{manualtheorem}{8.16}
	Let $p$ be a prime such that $p \equiv 1 \pmod{4}$. Then $p$ can be written as the sum of two squares of natural numbers.
\end{manualtheorem}

\begin{manualtheorem}{8.21}
	A natural number $n$ can be written as the sum of two squares of integers
	if and only if every prime congruent to $3 \pmod{4}$ in the unique prime
	factorization of $n$ occurs to an even power.
\end{manualtheorem}

\begin{manualtheorem}{8.22}
	If $(a,b,c)$ is a primitive Pythagorean triple, then $c$ is a product of primes, each of which is congruent to $1 \pmod{4}$.
\end{manualtheorem}

\begin{manualtheorem}{8.23}
	If the natural number $c$ is a product of primes, each of which is congruent to $1 \pmod{4}$, then there exist natural numbers $a$ and $b$ such that $(a,b,c)$ is a primitive Pythagorean triple.
\end{manualtheorem}

\subsection{Fermat's Last Theorem}

\begin{manualtheorem}{Fermat's Last Theorem}
	For natural numbers $n \ge 3$, there are no natural numbers $x, y, z$ such that $x^n + y^n = z^n$.
\end{manualtheorem}

\begin{manualtheorem}{8.24}

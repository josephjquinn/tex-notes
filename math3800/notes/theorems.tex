...
\begin{proof}
	Since $p \nmid a$, the integer $a$ has a multiplicative inverse modulo $p$,
	denoted $a^{-1}$. That is, $a a^{-1} \equiv 1 \pmod{p}$.
	...
	by $a^{-1}$ gives
	\[
		a^{-1}(a x^2 + b x + c) \equiv 0 \pmod{p}.
	\]
	Simplifying, we obtain
	\[
		x^2 + (a^{-1} b)x + (a^{-1} c) \equiv 0 \pmod{p}.
	\]
	Define
	\[
		b_0 \equiv a^{-1} b \pmod{p}, \qquad c_0 \equiv a^{-1} c \pmod{p}.
	\]
	...
	\[
		...
		x^2 + b_0 x + c_0 \equiv 0 \pmod{p}.
	\]
	Hence the two congruences have exactly the same set of solutions modulo $p$.
